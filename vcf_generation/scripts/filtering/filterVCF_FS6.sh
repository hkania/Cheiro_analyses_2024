...
##### filterVCF_FS6.sh: TO DO:
## Switch some vcftools commands to bcftools?

########################################################################################################################################
##### filterVCF_FS6.sh: SET-UP #####
########################################################################################################################################
...
BCFTOOLS=$SOFTWARE/bcftools-1.20/bcftools
VCFTOOLS=$SOFTWARE/vcftools-0.1.16/bin/bin/vcftools
MAWK=/datacommons/yoderlab/programs/mawk-1.3.4-20171017/mawk

SCR_QCVCF=/datacommons/yoderlab/users/hkania/cheiro/scripts/qc/qc_vcf.sh
...
VCF_FS8=$OUT_DIR/$OUTPUT_NAME.FS8.vcf

VCF_IN=$IN_DIR/$INPUT_NAME.vcf

[[ $SKIP_11 == TRUE ]] && VCF_OUT=$IN_DIR/$OUTPUT_NAME.vcf
[[ $SKIP_11 == FALSE ]] && VCF_OUT=$VCF_FS6
...
echo "##### filterVCF_FS6.sh: File with inds to remove: $INDFILE"
echo "##### filterVCF_FS6.sh: Filter inds by missing data: $FILTER_INDS_BY_MISSING"
printf "\n"
[[ $SKIP_4 == FALSE ]] && echo "##### filterVCF_FS6.sh: Filtering by minor allele count (MAC)..." && printf "\n"
...
printf "\n"
echo "##### filterVCF_FS6.sh: VCF - in: $VCF_IN"
ls -lh $VCF_IN
printf "\n"
echo "##### filterVCF_FS6.sh: VCF - out: $VCF_OUT"
echo "##### filterVCF_FS6.sh: VCF - TooHighDepth: $VCF_HIGHDEPTH"
echo "##### filterVCF_FS6.sh: VCF - FS7: $VCF_FS7"
echo "##### filterVCF_FS6.sh: VCF - FS8: $VCF_FS8"
printf "\n"
...
########################################################################################################################################
##### filterVCF_FS6.sh: STEP 0: PRE-SELECT SAMPLES #####
########################################################################################################################################
...
then
	...
	echo "##### filterVCF_FS6.sh: Keep command: $KEEP_COMMAND"
	printf "\n"
	echo "##### filterVCF_FS6.sh: Indfile contents:"
	cat $INDFILE
	
	VCF_STEP0=$ID.vcf
	printf "\n"
	echo "##### filterVCF_FS6.sh: Outputting to file $VCF_STEP0"
	printf "\n"
	...
	printf "\n"
	echo "##### filterVCF_FS6.sh: Number of individuals requested by indfile: $NR_INDS_REQUESTED"
	echo "##### filterVCF_FS6.sh: Number of individuals present in VCF: $NR_INDS_PRESENT"
	printf "\n"
	...
	echo "##### filterVCF_FS6.sh: Saving a copy of individual-selected file $ID.indSel.vcf..."
	cp $VCF_STEP0 $ID.indSel.vcf
	gzip -f $ID.indSel.vcf
...
########################################################################################################################################
##### filterVCF_FS6.sh: STEP 1: SELECT SNPS ONLY #####
########################################################################################################################################
if [ $SKIP_1 == TRUE ]
then
...
then
	...
	$JAVA -Xmx${MEM}g -jar $GATK3 -T SelectVariants -R $REF \
		-V $VCF_STEP0 \
		-o $ID.rawSNPs.tmp.vcf \
		-selectType SNP 

	printf "\n"
	echo "##### filterVCF_FS6.sh: Removing non-variable SNPs: $NVAR_ALLVARS"
	...
	printf "\n"
	echo "##### filterVCF_FS6.sh: Number of total variants (SNPs + indels): $NVAR_ALLVARS"
...
########################################################################################################################################
##### filterVCF_FS6.sh: STEP 2: ANNOTATE SNPS WITH ALLELE BALANCE #####
########################################################################################################################################
...
then
	...
	$JAVA -Xmx${MEM}G -jar $GATK3 -T VariantAnnotator \
		-R $REF \
		-V $VCF_STEP1 \
		-o $VCF_STEP2 \
		-A AlleleBalance
...
########################################################################################################################################
##### filterVCF_FS6.sh: STEP 3: FILTER BY MINIMUM DEPTH AND QUALITY #####
########################################################################################################################################
...
then
	echo -e "\n\n#####################################################################################################################"
	echo -e "##### filterVCF_FS6.sh: Step 3: filtering genotypes by minDP, meanDP, minQ, and potentially MAC....\n"
	date
	...
	$VCFTOOLS --vcf $VCF_STEP1 --recode --recode-INFO-all --minDP $DP_MIN --minQ $QUAL --out $ID.S3a
...
########################################################################################################################################
##### filterVCF_FS6.sh: STEP 4: FILTER BY MAC #####
########################################################################################################################################
...
########################################################################################################################################
##### filterVCF_FS6.sh: STEP 5: FILTER BY MISSING DATA - FIRST THREE ROUNDS #####
...
then
	...
	then
		...
		printf "\n\n"
		echo -e "##### filterVCF_FS6.sh: Number of individuals prior to filtering by missing data: $NIND_ALL"
		...
		printf "\n"
		echo -e "##### filterVCF_FS6.sh: Number of SNPs prior to filtering by missing data: $NVAR_S4"
...
########################################################################################################################################
##### filterVCF_FS6.sh: STEP 6: SOFT-FILTER SNPS WITH GATK #####
...
then
	...
	printf "\n"
	echo "##### filterVCF_FS6.sh: Nr of SNPs filtered by QD_lt2: $NFILT_QD"
...
then
	...
	printf "\n"
	echo "##### filterVCF_FS6.sh: Nr of SNPs filtered due to not being biallelic: $NFILT_S7"
...
########################################################################################################################################
##### filterVCF_FS6.sh: STEP 8: HARD-FILTER GATK-FILTERS WITH VCFTOOLS #####
...
then
	...
	## Run vcftools:
	$VCFTOOLS --vcf $VCF_STEP7 --remove-filtered-all --max-non-ref-af 0.99 --min-alleles 2 --recode --recode-INFO-all \
		--stdout > $VCF_STEP8
	...
	printf "\n"
	echo "##### filterVCF_FS6.sh: Total number of SNPs filtered by GATK filtering: $NFILT_S8"
...
########################################################################################################################################
##### filterVCF_FS6.sh: STEP 9: FILTER FOR QUAL/DEPTH RATIO AND MAX DEPTH #####
...
then
	...
	echo "##### filterVCF_FS6.sh: Number of SNPs filtered based on maximum mean depth: $NFILT_S9"
...
########################################################################################################################################
##### filterVCF_FS6.sh: STEP 10: FILTER BY MISSING DATA - FINAL ROUND #####
...
then
	...
	then
		...
		printf "\n"
		echo -e "##### filterVCF_FS6.sh: Number of individuals filtered in round 4: $NIND_EXCL_S10"
	...
	printf "\n"
	echo -e "##### filterVCF_FS6.sh: Number of SNPs filtered in round 4: $NFILT_S10"
	...
	printf "\n"
	echo -e "##### filterVCF_FS6.sh: Number of SNPs filtered when not allowing missing data: $NFILT_NOMISS"
...
## Final file:
echo -e "##### filterVCF_FS6.sh: Gzipping $VCF_STEP10..."
gzip -f $VCF_STEP10
echo "##### filterVCF_FS6.sh: Copying VCF_STEP10 $VCF_STEP10 to VCF_OUT $VCF_OUT..."
cp -f $VCF_STEP10.gz $VCF_OUT.gz
printf "\n"
echo "##### filterVCF_FS6.sh: Listing $VCF_OUT.gz..."
ls -lh $VCF_OUT.gz
printf "\n"

if [ $SKIP_11 == TRUE ]
then
	echo -e "##### filterVCF_FS6.sh: Skipping Step 11 (gzip etc)..."
fi

if [ $SKIP_11 == FALSE ]
then
	echo -e "\n\n#####################################################################################################################"
	echo -e "##### filterVCF_FS6.sh: Step 11: Gzip files and remove intermediate files..."
	
	if [ $SKIP_10 == FALSE ]
	then
		## Final file - minus last missing data filterings step ("FS7"):
		echo "##### filterVCF_FS6.sh: Gzipping FS7 vcf VCF_STEP9 $VCF_STEP9..."
		gzip $VCF_STEP9
		mv $VCF_STEP9.gz $VCF_FS7
		printf "\n"
		
		## Final file - NO missing data ("FS8"):
		echo "##### filterVCF_FS6.sh: Moving file with no missing data (FS8)..."
		mv $ID.noMiss.vcf.gz $VCF_FS8
		printf "\n"
	fi
	
	## Remove intermediate files:
	echo "##### filterVCF_FS6.sh: Gzipping $VCF_STEP1, $VCF_STEP2, $VCF_STEP3..." 
	gzip -f $VCF_STEP1
	gzip -f $VCF_STEP2
	gzip -f $VCF_STEP3
	
	echo "##### filterVCF_FS6.sh: Removing intermediate VCFs & tmp files..."
	rm -f $VCF_STEP4 $VCF_STEP5 $VCF_STEP6 $VCF_STEP7 $VCF_STEP8 $VCF_STEP9
...
then
	...
	RUN_VCF_SITESTATS=TRUE
	
	## Run for FS6:
	sbatch --mem ${MEM}G -p common,yoderlab,scavenger -o slurm.qcVCF.$OUTPUT_NAME.FS6 \
	$SCR_QCVCF $OUTPUT_NAME.FS6 $OUT_DIR $QC_DIR $RUN_BCF $RUN_BCF_BY_IND $RUN_VCF_SITESTATS
	
	## Run for FS7:
	sbatch --mem ${MEM}G -p common,yoderlab,scavenger -o slurm.qcVCF.$OUTPUT_NAME.FS7 \
	$SCR_QCVCF $OUTPUT_NAME.FS7 $OUT_DIR $QC_DIR $RUN_BCF $RUN_BCF_BY_IND $RUN_VCF_SITESTATS
	
	## Run for FS8:
	sbatch --mem ${MEM}G -p common,yoderlab,scavenger -o slurm.qcVCF.$OUTPUT_NAME.FS8 \
	$SCR_QCVCF $OUTPUT_NAME.FS8 $OUT_DIR $QC_DIR $RUN_BCF $RUN_BCF_BY_IND $RUN_VCF_SITESTATS
...
########################################################################################################################################
##### filterVCF_FS6.sh: REPORT ######
echo -e "\n\n#####################################################################################################################"
echo -e "\n##### filterVCF_FS6.sh: Giving final stats:"
...
echo "##### filterVCF_FS6.sh: Filter stats stored in $FILTSTATS"
