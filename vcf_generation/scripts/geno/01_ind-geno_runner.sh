...
## Scripts:
SCR_INDEXGENOME=/datacommons/yoderlab/users/jelmer/proj/wgs/scripts/misc/indexGenome.sh
SCR_REGIONSEL=/datacommons/yoderlab/users/hkania/cheiro/scripts/conversion/bam_selectregions.sh

SCR_ALIGN=/datacommons/yoderlab/users/hkania/cheiro/scripts/geno/bam1_align.sh
SCR_SORT=/datacommons/yoderlab/users/hkania/cheiro/scripts/geno/bam2_sort.sh
SCR_FILTERBAM=/datacommons/yoderlab/users/hkania/cheiro/scripts/geno/bam3_filter.sh
SCR_DEDUP=/datacommons/yoderlab/users/hkania/cheiro/scripts/geno/bam4_dedup.sh
SCR_REALIGN=/datacommons/yoderlab/users/hkania/cheiro/scripts/geno/bam5_realign.sh

SCR_VARDISC=/datacommons/yoderlab/users/hkania/cheiro/scripts/geno/gatk1_vardisc.sh
SCR_GENO=/datacommons/yoderlab/users/hkania/cheiro/scripts/geno/gatk2_jointgeno.sh
SCR_FILTER=/datacommons/yoderlab/users/hkania/cheiro/scripts/filtering/filterVCF_FS6.sh

SCR_QCVCF=/datacommons/yoderlab/users/hkania/cheiro/scripts/qc/qc_vcf.sh
SCR_QCBAM=/datacommons/yoderlab/users/hkania/cheiro/scripts/qc/qc_bam.sh
...
for ID_LONG in ${IDS_LONG[@]}; do
	...
	if [ $MAP == true ]; then
		...
		## Run mapping script:
		echo -e "\n###########################################################"
		echo "## Running mapping script for $ID_LONG..."
	...
	if [ $PROCESS_BAM_GENERAL == true ]; then
		...
		cp $BAM_DIR/intermed/$ID_LONG$SUFFIX_OUT.bam $BAM_DIR/final/$ID_LONG$SUFFIX_OUT.bam
...
if [ $VARDISC == true ] ; then
	...
	GATK_VERSION=gatk4
	SUFFIX_END=sort.MQ30.dedup.bam
	INPUT=/datacommons/yoderlab/users/hkania/cheiro/results/bam/intermed/$ID_SHORT$SUFFIX_END
