...
# SET-UP -----------------------------------------------------------------------
## Scripts:
SCR_GENO=/datacommons/yoderlab/users/hkania/cheiro/scripts/geno/2_gatk/gatk2_jointgeno.sh
SCR_MERGEVCFS=/datacommons/yoderlab/users/hkania/cheiro/scripts/geno/2_gatk/gatk3_mergevcfs.sh
SCR_FILTER=/datacommons/yoderlab/users/hkania/cheiro/scripts/geno/03_filterVCF_FS6_runner.sh
...
sbatch -p yoderlab,common,scavenger -o slurm.filterVCFpop.$FILE_ID.$OUTPUT_NAME --mem "$MEM_JOB"G --job-name=jgeno.pip."$FILE_ID" --dependency=singleton \
